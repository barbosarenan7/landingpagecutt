...
> **Reescrita aplicada.** A URL não mudou e o rótulo do menu continua "Produção audiovisual". A página passou do template de serviço (6 seções) para o de landing BOFU (11 seções). **Tem um bloco `TROCAR ANTES DE PUBLICAR` visível na página**, com os dados da estrutura audiovisual que faltam.
...
Quase toda empresa já tentou resolver vídeo por conta própria. Comprou um bom celular, contratou um freelancer por diária, gravou um lote de material num sábado. O primeiro mês sai bem. O terceiro já não sai, e no sexto ninguém lembra por que parou. Não foi falta de equipamento, foi falta de direção e de uma estrutura que aguente a rotina.

O outro sintoma é mais silencioso: o material sai, mas não parece a sua empresa. Cada peça tem uma cor, um enquadramento e um ritmo diferente, porque cada uma foi feita por uma pessoa diferente. O público não consegue nomear o problema, mas percebe. E marca que parece improvisada tem menos margem para cobrar caro.
...
pré-renderizado. Confirmado no HTML cru de `dist/producao-audiovisual-volta-redonda/index.html`:

| Bloco | Origem |
| --- | --- |
| `ProfessionalService` + `LocalBusiness` | endereço, telefone e área atendida |
| `Service` | nome e tipo do serviço desta página |
| `BreadcrumbList` | Início → Produção audiovisual |
